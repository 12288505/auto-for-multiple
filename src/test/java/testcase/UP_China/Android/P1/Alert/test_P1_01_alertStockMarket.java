...
public class test_P1_01_alertStockMarket {
	...
	public void setUp() {
		...
		up = new UP_Android();
		up.log("开始测试：验证推送消息-股价涨停预警被触发");
		up.openApp();
	}

	/**
	 * 测试名称：01 验证推送消息-股价涨停预警被触发
	 * [前提条件]：
	 * 1、设置接受的股价涨预警；
	 * 2、当前股价达到预警条件
	 * 
	...
	public void testAlertStockMarket() {
		...
		up.clickOn("提交");
		up.verifyIsShown("标题");
		// double newPrice = Double.parseDouble(up.getValueOf("现价"));
		//
		// // while (newPrice < alertHighPrice && newPrice > alertLowPrice) {
		// // newPrice = Double.parseDouble(up.getValueOf("现价"));
		// // up.log("现价没有到预警范围，等待5秒");
