...
public class test_P1_04_weibozhanghao {
	...
	public void setUp() {
		...
		up = new UP_Android();
		up.log("开始测试：微博已安装已登录第一次绑定登录------微博需要登录验证现在无法做");
		up.openApp();
	}

	/**
	 * 测试名称：04 微博已安装已登录第一次绑定登录
	 * [前提条件]：
	...
	public void testWeibozhanghao() {
		...
		up.back();
		up.clickOn("登录");
		up.setValueTo("输入框", up.getValueOf("验证码"));
		up.clickOn("确定");
