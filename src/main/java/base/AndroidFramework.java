...
import tools.AndroidMonitor;
import tools.CreateExcel;
import tools.GeneralMethods;
import base.core.UiFramework;

public abstract class AndroidFramework extends UiFramework {

	private CreateExcel ce = new CreateExcel();
	...
	protected AndroidFramework() {

		super();
	}

	protected AndroidFramework(String SUT) {
	...
	protected void startAppiumDriver(String app_apk) {
		...
		try {
			...
			if (!app.exists()) {
				...
				app = new File(classpathRoot, getProperty("app.path"));
	...
	public void monkeyTest(int count) {
		...
		if (nStr.isEmpty()) {
			AndroidMonitor.Monkey(getProperty("app.package"), String.valueOf(count));
			log("随机事件次数：" + count);
		} else {
			int times = Integer.parseInt(nStr);
			AndroidMonitor.Monkey(getProperty("app.package"), String.valueOf(times));
			log("随机事件次数：" + times);
		}
	}
